...
function PeoplePage() {
    ...
    useEffect(() => {
        ...
        const filtered = people.filter(p => p.name.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    const lastPersonRef = useCallback((node: HTMLDivElement) => {
        if (loading) return;
                ...
                <div id="soundbite-grid-container">
                    <div className="soundbite-grid">
                        {visiblePeople.map((person, index) => (
                            <div
                                key={person.name}
                                ref={index === visiblePeople.length - 1 ? lastPersonRef : undefined}
                            >
